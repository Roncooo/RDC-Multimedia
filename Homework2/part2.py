...
def perform_pings_and_save_into_file(K: int, L_byte_step: int, output_file: str, target_name: str, max_threads: int=10):
    with ThreadPoolExecutor(max_workers=max_threads) as executor:
        futures = []
        # Submit tasks to the executor
        for L_byte in range(min_L_byte,max_L_byte,L_byte_step):
...
K = 30 # number of packets sent for each L_byte value
target_name = "atl.speedtest.clouvider.net"
output_file = "lots_of_pings.txt"
# perform_pings_and_save_into_file(K=K, L_byte_step=4, output_file=output_file, target_name=target_name)
